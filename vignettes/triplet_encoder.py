...
def filter(dat):
    counts = {x: max(int(dat[x].shape[0]/20), 500) for x in dat.keys()}
...
if __name__ == '__main__':
    ...
    # train model
    model = flexynesis.train_model(multi_triplet_network, triplet_train_dataset, n_epoch, batch_size, val_size = 0.1) 
