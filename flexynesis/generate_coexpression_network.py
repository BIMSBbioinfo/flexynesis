...
def calculate_pairwise_correlations(expr_df, method='spearman', device=None):
    """
    DEPRECATED: Use build_network_streaming for large datasets to save memory.
    Calculate pairwise gene correlations using GPU acceleration.
    ...
    Returns:
        DataFrame: Correlation matrix (genes × genes)
    """
    # Auto-detect device if not specified
    if device is None:
        if torch.cuda.is_available():
    ...
    # Use PyTorch's corrcoef function in batches
    batch_size = 1000  # Process 1000 genes at a time
    
    print(f"Computing correlations in batches of {batch_size}...")
    corr_matrix = torch.zeros((n_genes, n_genes), device=device)
    ...
    with tqdm(total=n_genes, desc="Computing correlations") as pbar:
        for i in range(0, n_genes, batch_size):
            end_i = min(i + batch_size, n_genes)
            batch = data_normalized[i:end_i]
            # Compute correlation: (batch @ data_normalized.T) / n_samples
            corr_matrix[i:end_i] = torch.mm(batch, data_normalized.T) / data.shape[1]
            pbar.update(end_i - i)
    
    # Move back to CPU and convert to pandas DataFrame
...
def build_network_from_correlations(
    corr_matrix,
    min_correlation=0.3,
    top_k=10,
    ...
    """
    Build network edge list from correlation matrix.
    
    Args:
        corr_matrix: Gene × gene correlation matrix
        min_correlation: Minimum absolute correlation to include
        top_k: Keep top K neighbors per gene (based on absolute correlation)
        remove_self_loops: Remove self-correlations (gene-gene)
        remove_duplicates: Remove duplicate edges (A-B and B-A)
        
    Returns:
        DataFrame with columns: GeneA, GeneB, Score
    """
    genes = corr_matrix.index
    edges = []
    
    print(f"Building network (min |r| = {min_correlation}, top {top_k} per gene)...")
    
    for gene in tqdm(genes, desc="Processing genes"):
        # Get correlations for this gene
        gene_corr = corr_matrix.loc[gene]
        
        # Remove self-correlation if requested
        if remove_self_loops:
            gene_corr = gene_corr[gene_corr.index != gene]
        
        # Filter by threshold
        filtered = gene_corr[gene_corr.abs() >= min_correlation]
        
        # Keep top K by absolute correlation
        if len(filtered) > top_k:
            top_neighbors = filtered.abs().nlargest(top_k).index
            filtered = gene_corr.loc[top_neighbors]
        
        # Add edges
        for neighbor in filtered.index:
            edges.append({
                'GeneA': gene,
                'GeneB': neighbor,
                'Score': abs(corr_matrix.loc[gene, neighbor])  # Use absolute value
            })
