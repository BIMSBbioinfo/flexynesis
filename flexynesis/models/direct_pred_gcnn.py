...
class DirectPredGCNN(pl.LightningModule):
    ...
    def model_forward(self, *args):
        xs = list(args[:-4])
        edge_index_arg = args[-4]
        batch_arg = args[-3]
        ptr_arg = args[-2]
        target_var = args[-1]
        inputs = [
            Batch(x=_x.view(_x.nelement(), -1), edge_index=_edge_index, batch=_batch, ptr=_ptr)
            for _x, _edge_index, _batch, _ptr in zip(xs, edge_index_arg, batch_arg, ptr_arg)
        ]
    ...
    def compute_feature_importance(self, dataset, target_var, steps=5, batch_size = 32):
        ...
        dataloader = DataLoader(dataset, batch_size, num_workers=0, pin_memory=True, shuffle=False)
        ...
        attributions = [[[] for __ in range(len(dataset[0][0]))] for _ in range(num_class)]
        for i, batch in enumerate(dataloader):
            dat, _ = batch
            dat = {k: v.to(device) for k, v in dat.items()}

            xs = [x.x.view(x.batch_size, x.x.size(0) // x.batch_size) for x in dat.values()]
            edge_indices = [x.edge_index for x in dat.values()]
            batch_ = [x.batch for x in dat.values()]
            ptr = [x.ptr for x in dat.values()]

            inputs = tuple(xs)
            edge_index_arg = tuple(edge_indices)
            batch_arg = tuple(batch_)
            ptr_arg = tuple(ptr)
            additional_forward_args = (edge_index_arg, batch_arg, ptr_arg, target_var)
            ...
            for a, b in zip(attributions, batch_attributions):
                for aa, bb in zip(a, b):
                    aa.append(bb.cpu())

        # Move computations back to the CPU for further processing
        abs_attr = [[torch.abs(torch.cat(a, dim=0)).cpu() for a in attr_class] for attr_class in attributions]
