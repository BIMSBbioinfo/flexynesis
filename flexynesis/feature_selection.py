...
def remove_redundant_features(X, laplacian_scores, threshold, topN=None):
    ...
    ranked_indices = np.argsort(
        laplacian_scores
    )  # Assuming minimizing laplacian_scores
    selected_features = []
    redundant_features = {}
...
def filter_by_laplacian(X, layer, k=5, t=None, topN=100, correlation_threshold=0.9):
    ...
    """
    print(
        "[INFO] Implementing feature selection using laplacian score for layer:",
        layer,
        "with ",
        X.shape[1],
        "features",
    ...
    if topN >= X.shape[1]:
        print(
            "[INFO] No feature selection applied. Returning original matrix. Demanded # of features is ",
    ...
    if correlation_threshold < 1:
        ...
        selected_features, redundant_features_df = remove_redundant_features(
            X[X.columns[selected_feature_indices]],
            scores[selected_feature_indices],
        ...
        if not redundant_features_df.empty:
            ...
            feature_log = pd.merge(
                feature_log, redundant_features_df, on="feature", how="outer"
            )
